...
public:
    Pokemon pokemon;
    int level, IV, HP, atk, def, s_atk, s_def, speed, accuracy, evasion;
...
// spawn pokemon
void spawn_pokemon(pokemon_gen *p)
{
    int dis, i = 0;

    p->pokemon = data.pokemon_list[rand() % (sizeof(data.pokemon_list) / sizeof(data.pokemon_list[0]))];
    p->is_shiny = (rand() % 8192 == 0); // might need to change to (rand() % 8192 == 0 ? true : false)
    p->isMale = (rand() % 2 == 0);
    ...
    if (dis <= 200)
    {
        ...
        //     p->level = 1;
        // else
        //     p->level = (rand() % (dis / 2)) + 1;
        p->level = (rand() % ((dis < 3 ? 2 : dis) / 2)) + 1;
    }
    else
    {
        // do
        // {
        //     p->level = rand() % 101;
        // } while (p->level < (dis - 200) / 2);

        p->level = (rand() % (101 - (dis - 200) / 2) ? ((dis - 200) / 2) : 1) + ((dis - 200) / 2) ? ((dis - 200) / 2) : 1;
    }

    // select random moves for pokemon
    // while (index < (int)move_list.size() && pokemon_moves_list[index].pokemon_id != pokemon.species_id)
    //     index++;
    // i = index;
    while (i < (int)(sizeof(data.pokemon_moves_list) / sizeof(data.pokemon_moves_list[0])) && data.pokemon_moves_list[i].pokemon_id <= p->pokemon.species_id)
    {
        if (data.pokemon_moves_list[i].pokemon_id == p->pokemon.species_id && data.pokemon_moves_list[i].level >= p->level / 2 && data.pokemon_moves_list[i].level <= p->level && data.pokemon_moves_list[i].pokemon_move_method_id == 1
            // && data.pokemon_moves_list[i].version_group_id == 19
        )
        {
            int temp = 0, j, max;
            while (data.pokemon_moves_list[i].pokemon_id == p->pokemon.species_id && data.pokemon_moves_list[i].move_id != data.move_list[temp].id)
                temp++;

            if ((int)p->move_set.size() > 0)
            {
                max = p->move_set.size();
                for (j = 0; j < max; j++)
                {
                    if (p->move_set[j].identifier == data.move_list[temp].identifier)
                        break;

                    if (j == (int)p->move_set.size() - 1 && p->move_set[j].identifier != data.move_list[temp].identifier)
                        p->move_set.push_back(data.move_list[temp]);
                }
            }
            else
                p->move_set.push_back(data.move_list[temp]);
    ...
    if (p->move_set.size() <= 1)
    {
        i = 0;
        while (i < (int)(sizeof(data.pokemon_moves_list) / sizeof(data.pokemon_moves_list[0])) && data.pokemon_moves_list[i].pokemon_id <= p->pokemon.species_id)
        {
            if (data.pokemon_moves_list[i].pokemon_id == p->pokemon.species_id && data.pokemon_moves_list[i].level <= p->level && data.pokemon_moves_list[i].pokemon_move_method_id == 1
                // && data.pokemon_moves_list[i].version_group_id == 19
            )
            {
                int temp = 0, j, max;
    ...
    while (p->move_set.size() > 2)
    {
        p->move_set.erase(p->move_set.begin() + (rand() % p->move_set.size()));
    }
    if (rand() % (101 - p->level) > p->level && p->move_set.size() > 1)
        p->move_set.erase(p->move_set.begin() + 1);
    // complete move selection

    i = 0;
    while (i < (int)(sizeof(data.stats_list) / sizeof(data.stats_list[0])) && data.stats_list[i].pokemon_id <= p->pokemon.species_id)
    {
        if (data.stats_list[i].pokemon_id == p->pokemon.species_id)
        {
            switch (data.stats_list[i].stat_id)
            {
            case 1:
                p->HP = (int)((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100.0 + p->level + 10;
                break;
            case 2:
                p->atk = (int)((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100.0 + 5;
                break;
            case 3:
                p->def = (int)((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100.0 + 5;
                break;
            case 4:
                p->s_atk = (int)((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100.0 + 5;
                break;
            case 5:
                p->s_def = (int)((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100.0 + 5;
                break;
            case 6:
                p->speed = (int)((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100.0 + 5;
                break;
            // case 7:
            //     p->accuracy = ((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100 + 5;
            //     break;
            // case 8:
            //     p->evasion = ((data.stats_list[i].base_stat + p->IV) * 2 * p->level) / 100 + 5;
            //     break;
            default:
                break;
            }
...
void initiate_pokemon_battle(pokemon_gen *p)
{
    int i, j;
    for (j = 2; j < yN; j++)
    {
    ...
    std::string name = p->pokemon.identifier.data();
    toupper(name[0]);

    mvprintw(0, 0, "Wild %s Appear!     Press ESC to return.", name.data());

    i = 0, j = 2;
    mvprintw(j++, xN / 2 - 25, "Name:            %s", name.data());
    mvprintw(j++, xN / 2 - 25, "Gender:          %s", p->isMale ? "Male" : "Female");
    mvprintw(j++, xN / 2 - 25, "Level:           %d", p->level);
    mvprintw(j++, xN / 2 - 25, "IV:              %d", p->IV);
    mvprintw(j++, xN / 2 - 25, "Health Point:    %d", p->HP);
    mvprintw(j++, xN / 2 - 25, "Attack:          %d", p->atk);
    mvprintw(j++, xN / 2 - 25, "Special Attack:  %d", p->s_atk);
    mvprintw(j++, xN / 2 - 25, "Defence:         %d", p->def);
    mvprintw(j++, xN / 2 - 25, "Special Defence: %d", p->s_def);
    mvprintw(j++, xN / 2 - 25, "Speed:           %d", p->speed);
    ...
    for (i = 0; i < (int)p->move_set.size(); i++)
        mvprintw(j++, xN / 2 - 18, "- %s (%d)", p->move_set[i].identifier.data(), p->move_set[i].id);
    ...
    else
    {
        ...
        if (findTileCost(ter, player) != INT_MAX)
        {
            ...
            if (ter == grass_1 || ter == grass_2 || ter == grass_3 || ter == grass_4)
            {
                if (rand() % 10 == 0)
                {
                    pokemon_gen wild_pokemon;
                    spawn_pokemon(&wild_pokemon);
                    initiate_pokemon_battle(&wild_pokemon);
                }
            }
            Bworld.pc.pos[dim_x] += all_dirs[dir][dim_x];
            Bworld.pc.pos[dim_y] += all_dirs[dir][dim_y];
            ...
            do
            {
                ...
                case 'f':
                    ...
                    if (Bworld.currMap == NULL)
                    {
                        ...
                        trainer_init(Bworld.currMap, numTrainer);
                        Bworld.pc.currCost = 0;
                    }
                    else
                    {
