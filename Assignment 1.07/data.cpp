...
void parse_pokemon(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->pokemon_list[i++] = Pokemon(id, identifier, species_id, height, weight, base_experience, order, is_default);
    }
...
void parse_pokemon_move(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->pokemon_moves_list[i++] = (pokemon_move(pokemon_id, version_group_id, move_id, pokemon_move_method_id, level, order));
    }
...
void parse_pokemon_species(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->species_list[i++] = (pokemon_species(id, identifier, generation_id, evolves_from_species_id, evolution_chain_id, color_id, shape_id, habitat_id, gender_rate, capture_rate, base_happiness, is_baby, hatch_counter, has_gender_differences, growth_rate_id, forms_switchable, is_legendary, is_mythical, order, conquest_order));
    }
...
void parse_pokemon_stats(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->stats_list[i++] = (pokemon_stats(pokemon_id, stat_id, base_stat, effort));
    }

    // for (int i = 0; i < (int)data->stats_list.size(); i++)
    // {
    //     data->stats_list[i].print();
    // }
}

void parse_type_names(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->type_names_list[i++] = (type_names(type_id, name, local_language_id));
    }
...
void parse_experience(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->experience_list[i++] = (experience(growth_rate_id, level, exp));
    }
...
void parse_move(data_t *data, std::ifstream *file)
{
    ...
    while (getline(*file, line))
    {
        ...
        data->move_list[i++] = (moves(id, identifier, generation_id, type_id, power, pp, accuracy, priority, target_id, damage_class_id, effect_id, effect_chance, contest_type_id, contest_effect_id, super_contest_effect_id));
    }
...
void parse(data_t *data, std::string *fname)
{
    ...
    if (*fname == "pokemon.csv")
        parse_pokemon(data, &file);
    else if (*fname == "pokemon_moves.csv")
        parse_pokemon_move(data, &file);
    else if (*fname == "pokemon_species.csv")
        parse_pokemon_species(data, &file);
    else if (*fname == "pokemon_stats.csv")
        parse_pokemon_stats(data, &file);
    else if (*fname == "type_names.csv")
        parse_type_names(data, &file);
    else if (*fname == "experience.csv")
        parse_experience(data, &file);
    else if (*fname == "moves.csv")
        parse_move(data, &file);
    else if (*fname == "pokemon_stats.csv")
        parse_move(data, &file);
    else
...
void parseAll(data_t *data)
{
    ...
    std::vector<std::string> csvNames = {"pokemon.csv", "pokemon_moves.csv", "pokemon_species.csv", "pokemon_stats.csv", "type_names.csv", "experience.csv", "moves.csv"};
    std::ifstream file;
    file.open(tempString, std::ios::in);
    if (!file)
    {
