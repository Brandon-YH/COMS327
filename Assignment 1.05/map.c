...
int checkValidMove(map_t *m, info_t *info, dir_t dir)
{
    ...
    terrain_t nextTer = m->mp[info->pos[dim_y] + all_dirs[dir][dim_y]][info->pos[dim_x] + all_dirs[dir][dim_y]];
...
void shiftTrainer(map_t *m, info_t *info, dir_t dir, int counter)
{
    ...
    if (findTileCost(m->mp[info->pos[dim_y] + all_dirs[dir][dim_y]][info->pos[dim_x] + all_dirs[dir][dim_x]], info->trainer) != INT_MAX && (m->npc_mp[info->pos[dim_y] + all_dirs[dir][dim_y]][info->pos[dim_x] + all_dirs[dir][dim_x]] == NULL))
    {
