...
Pokemon::Pokemon(int level) : level(level)
{
    ...
    int growthRate = species_list[pokemon_species_index].growth_rate_id;
    int index = ((growthRate - 1) * 100) + level - 1;
    effective_stat[stat_exp] = experience_list[index].exp;
    effective_stat[stat_currExp] = 0;
}
...
int Pokemon::get_currExp() const
{
    return effective_stat[stat_currExp];
}

int Pokemon::get_maxExp() const
{
    return effective_stat[stat_exp];
}
...
void Pokemon::addExp(int exp){
    if (effective_stat[stat_exp] <= effective_stat[stat_currExp] + exp){
        levelUp(exp);
    } else {
        effective_stat[stat_currExp] += exp;
    }
}

void Pokemon::levelUp(int exp){
    effective_stat[stat_currExp] = effective_stat[stat_currExp] + exp - effective_stat[stat_exp];
    level++;
    ...
    while (i < (int)(sizeof(stats_list) / sizeof(stats_list[0])) && stats_list[i].pokemon_id <= pokemon.species_id)
    {
        if (stats_list[i].pokemon_id == pokemon.species_id)
        {
            ...
            {
            case 1:
                effective_stat[stat_hp] = (int)((stats_list[i].base_stat + IV) * 2 * level) / 100.0 + level + 10;
                break;
            case 2:
                effective_stat[stat_atk] = (int)((stats_list[i].base_stat + IV) * 2 * level) / 100.0 + 5;
                break;
    ...
    if (effective_stat[stat_exp] <= effective_stat[stat_currExp])
        levelUp(effective_stat[stat_currExp]);
}